...
import io.project.RegistrationForWorkshops.model.State;
import io.project.RegistrationForWorkshops.model.Subject;
import io.project.RegistrationForWorkshops.model.UserContainer;
import io.project.RegistrationForWorkshops.model.UserDAO;
...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            switch (messageText) {
                ...
                case "/cancel" -> setState(userId, State.INITIAL);
                case "/register" -> registerUser(userId, update.getMessage().getChatId());
                case "/slots" -> showAvailableSlots(userId, chatId);
                default -> { }
            }
            switch (userState) {
                case EXPECT_NAME -> setNameAndAskGroup(userId, chatId, messageText);
                case EXPECT_GROUP -> setGroupAndAskEmail(userId, chatId, messageText);
                case EXPECT_EMAIL -> setEmailAndFinishRegistration(userId, chatId, messageText);
                case EXPECT_ID -> makeRecordInSheet(userId, chatId, messageText);
                default -> { }
            }
        }
        if (update.hasCallbackQuery()) {
            ...
            if (callbackData.equals("Sign Up")) {
                ...
                replyWithFilteredSlots(callbackData, chatId, messageId);
            }
        }
    }
    ...
    public static String getColumnName(int columnNumber) {
        ...
        int copyNumber = columnNumber;
        while (copyNumber > 0) {
            int modulo = (columnNumber - 1) % 26;
            columnName.insert(0, (char) ('A' + modulo));
            copyNumber = (copyNumber - modulo) / 26;
        }
    ...
    private void replyWithFilteredSlots(String subject, long chatId, long messageId) {
        String answer = SlotContainer.getAvailableFilteredBySubject(subject).stream()
                .map(Object::toString)
    ...
    private void showAvailableSlots(long userId, long chatId) {
        List<SlotDAO> relevantSlots = SlotContainer.getAvailableSlots();
